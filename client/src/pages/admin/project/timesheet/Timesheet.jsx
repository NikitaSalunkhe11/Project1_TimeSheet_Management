...
const StyledTable = () => {
  ...
  const handleFormSubmit = async (e) => {
    e.preventDefault();
    const assignedDate = taskData.assignedDate.format('YYYY-MM-DD'); // Ensure the assignedDate is in the correct format
  
    console.log('Task data submitted:');
    try {
      const response = await ApiInstance.post('/api/tasks', {
        ...taskData,
        assignedDate,
      });
      ...
      setTasks((prevTasks) => {
        const updatedTasks = [...prevTasks, newTask]; // Append the new task to the existing tasks
        return updatedTasks;
      });
